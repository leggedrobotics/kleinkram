...
import {clearAllData, db, get_jwt_token, get_user_from_db, mock_db_user,} from './utils/database_utils';
...
describe('Access Control', () => {
    ...
    test('non leggedrobotics.com email is not added to default group', async () => {
        ...
        const external_uuid = await mock_db_user('external-user@third-party.com')
        ...
        const personal_group = accessGroups.filter((group: AccessGroup) => group.personal === true);
        expect(personal_group.length).toBe(1);

        // one access group should have the default uuid
        const default_group = accessGroups.filter((group: AccessGroup) => group.uuid === default_groups[0]);
    ...
    test('reject corrupted JWT token', async () => {
        const jwt = require('jsonwebtoken');
        const token = jwt.sign({user: 'this-is-not-a-valid-uuid'}, process.env.JWT_SECRET);
    ...
    test('if user with leggedrobotics email is allowed to create new project', async () => {
        ...
        const project = await projectRepository.findOneOrFail({where: {uuid: project_uuid}});
    ...
    test('if user with admin role is allowed to create new project', async () => {
        const user_id = await mock_db_user('John Doe', 'test@example.com', UserRole.ADMIN);
        const user = await get_user_from_db(user_id);
    ...
    test('if user with admin role can view all projects', async () => {
        ...
        const user_id = await mock_db_user('some-admin@third-party.com', undefined, UserRole.ADMIN);
        const user = await get_user_from_db(user_id);

        const token = await get_jwt_token(user);
        const res = await fetch(`http://localhost:3000/project?take=11&skip=0&sortBy=name&descending=false`, {
            method: 'GET',
            headers: {
                cookie: `authtoken=${token}`,
            },
    ...
    test('the creator of a project has delete access to the project', async () => {
        ...
        const projectRepository = db.getRepository('Project');
        let project = await projectRepository.findOneOrFail({where: {uuid: project_uuid}});
        expect(project.name).toBe('Test Project');

        // delete the project using the API
        const token = await get_jwt_token(user);
        const res = await fetch(`http://localhost:3000/project/delete?uuid=${project_uuid}`, {
            method: 'DELETE',
            headers: {
                cookie: `authtoken=${token}`,
            },
    ...
    test('if user with admin role can delete any project', async () => {
        ...
        const creator_user_id = await mock_db_user('creator@leggedrobotics.com');
        const userRepository = db.getRepository(User);
        const user = await userRepository.findOne({where: {uuid: creator_user_id}});
        ...
        const admin_user_id = await mock_db_user('third-party-admin@test.com', undefined, UserRole.ADMIN);
        const third_party_user = await userRepository.findOne({where: {uuid: admin_user_id}});
    ...
    test('third party user cannot view any project by default', async () => {
        const interal_user_id = await mock_db_user('some-interal-user@leggedrobotics.com');
        ...
        const external_user_id = await mock_db_user('some-external-user@third-party.com');
        ...
        const res2 = await fetch(`http://localhost:3000/project/one?uuid=${project_uuid}`, {
            method: 'GET',
            headers: {
        ...
        const res = await fetch(`http://localhost:3000/project?take=10&skip=0&sortBy=name&descending=false`, {
            method: 'GET',
            headers: {
                cookie: `authtoken=${external_user_token}`,
            },
    ...
    test('internal user have read only access by default', async () => {
        const interal_user_id = await mock_db_user('internal-1@leggedrobotics.com');
        ...
        const second_user_id = await mock_db_user('internal-2@leggedrobotics.com');
        const second_user = await get_user_from_db(second_user_id);

        // check view access
        const token = await get_jwt_token(second_user);
        const res = await fetch(`http://localhost:3000/project/one?uuid=${project_uuid}`, {
            method: 'GET',
            headers: {
                cookie: `authtoken=${token}`,
            },
        ...
        const res2 = await fetch(`http://localhost:3000/project/update?uuid=${project_uuid}`, {
            method: 'PUT',
            headers: {
        ...
        const res3 = await fetch(`http://localhost:3000/project/delete?uuid=${project_uuid}`, {
            method: 'DELETE',
            headers: {
                cookie: `authtoken=${token}`,
            },
