...
const createService = () => {
  ...
  const projectArchiveNameQueryBuilder = {
    where: jest.fn(),
    andWhere: jest.fn(),
    getExists: projectExistsInTx,
  };
  projectArchiveNameQueryBuilder.where.mockReturnValue(
    projectArchiveNameQueryBuilder,
  );
  projectArchiveNameQueryBuilder.andWhere.mockReturnValue(
    projectArchiveNameQueryBuilder,
  );
...
describe("ProjectService.migrateProject", () => {
  ...
  test("throws conflict when source contains names that only differ by letter case", async () => {
    const {
      service,
      projectFindOneInTx,
      projectExistsInTx,
  ...
  test("checks archive project names with exact case-insensitive equality", async () => {
    const {
      service,
      projectFindOneInTx,
      projectExistsInTx,
    ...
    projectFindOneInTx.mockResolvedValue({});
    projectExistsInTx.mockResolvedValue(false);
    missionFindInTx.mockResolvedValueOnce([]).mockResolvedValueOnce([]);

    await service.migrateProject({
      sourceProjectUUID,
      targetProjectUUID,
      archiveSourceProjectAs: "legacy_source",
    });

    expect(projectArchiveNameQueryBuilder.where).toHaveBeenCalledWith(
      "project.uuid != :sourceProjectUUID",
      { sourceProjectUUID },
    );
    expect(projectArchiveNameQueryBuilder.andWhere).toHaveBeenCalledWith(
      "LOWER(project.name) = LOWER(:archiveName)",
      {
        archiveName: "legacy_source",
      },
    );
  });
