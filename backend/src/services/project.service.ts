...
export class ProjectService {
  ...
  private async _getProjectSizes(
    ...
    const rawResults = await this.projectRepository
      ...
      .leftJoin("project.missions", "mission", "mission.deletedAt IS NULL")
  ...
  async create(project: CreateProject, auth: AuthHeader): Promise<ProjectDto> {
    const exists = await this.projectRepository.exists({
      where: { name: ILike(project.name) },
    });
    if (exists) {
      throw new ConflictException("Project with that name already exists");
    }
    ...
    const transactedProject = await this.dataSource.transaction(
      async (manager: EntityManager): Promise<ProjectEntity> => {
        const savedProject = await manager.save(ProjectEntity, newProject);
  ...
  async deleteProject(uuid: string): Promise<void> {
    await this.dataSource.transaction(async (transactionalEntityManager) => {
      ...
      const deleteResult = await transactionalEntityManager.softDelete(
        ProjectEntity,
        {
          uuid,
        },
      ...
      await this.dataSource.transaction(async (manager) => {
        ...
        if (archiveSourceProjectAs !== undefined) {
          const archiveNameTaken = await projectRepository
            ...
            .andWhere("LOWER(project.name) = LOWER(:archiveName)", {
              archiveName: archiveSourceProjectAs,
            })
            .getExists();
          if (archiveNameTaken) {
            throw new ConflictException(
        ...
        const normalizedSourceMissionNames = sourceMissions.map((mission) =>
          mission.name.toLowerCase(),
        );
        if (
          new Set(normalizedSourceMissionNames).size !==
          normalizedSourceMissionNames.length
        ) {
          throw new ConflictException(
            "Source project contains mission names that differ only by letter case",
          );
        }
        const targetMissionNames = new Set(
          targetMissions.map((mission) => mission.name.toLowerCase()),
        );
        const collidingMissionNames = sourceMissions
          .map((mission) => mission.name)
          .filter((name) => targetMissionNames.has(name.toLowerCase()));
  ...
  async getDefaultRights(auth: AuthHeader): Promise<DefaultRights> {
    ...
    const defaultRights: (DefaultRightDto | undefined)[] = await Promise.all(
      creator.memberships
        ...
        .map(async (right) => {
          ...
          let memberCount = 1;
          let _rights: AccessGroupRights | undefined = AccessGroupRights.WRITE;
          ...
          switch (right.type) {
            case AccessGroupType.AFFILIATION: {
              ...
              memberCount = await this.userService.getMemberCount(right.uuid);
              break;
            }
            case AccessGroupType.PRIMARY: {
