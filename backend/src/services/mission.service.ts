...
export class MissionService {
  ...
  async create(
    ...
    await Promise.all(
      Object.entries(createMission.tags).map(async ([tagTypeUUID, value]) => {
        return this.tagService.addTagType(newMission.uuid, tagTypeUUID, value);
      }),
    );
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        ...
        if (missions.length !== uniqueMissionUUIDs.length) {
          const foundUUIDs = new Set(missions.map((mission) => mission.uuid));
        ...
        if (
          new Set(normalizedTargetNames).size !== normalizedTargetNames.length
        ) {
          throw new ConflictException(
        ...
        const existingTargetMissionNames = new Set(
          targetMissions.map((mission) => mission.name.toLowerCase()),
        );
        const collidingMissionNames = Array.from(
          targetMissionNames.values(),
        ).filter((name) => existingTargetMissionNames.has(name.toLowerCase()));
        ...
        for (const mission of fullyLoadedMissions) {
          const targetMissionName = targetMissionNames.get(mission.uuid);
  ...
  async updateTags(
    ...
    await Promise.all(
      Object.entries(tags).map(async ([tagTypeUUID, value]) => {
        ...
        if (tag) {
          return this.tagService.updateTagType(missionUUID, tagTypeUUID, value);
        }
        return this.tagService.addTagType(missionUUID, tagTypeUUID, value);
      }),
    );
  }
  ...
  async updateName(uuid: string, name: string): Promise<MissionEntity | null> {
    const exists = await this.missionRepository.exists({
      where: { name: ILike(name), uuid: Not(uuid) },
    });
    if (exists) {
      throw new ConflictException("Mission with that name already exists");
    }
