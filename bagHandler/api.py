...
async def upload_bag(file: UploadFile = File(...)) -> Any:
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=".bag") as temp_file:
        ...
        t_temp_time = time.time()
        cmd = ["mcap", "convert", temp_file_path, mcap_path]
        result = subprocess.run(cmd, capture_output=True, text=True)
        t_mcap_time = time.time()
        print(f"temptime: {t_temp_time-t_start}, mcaptime: {t_mcap_time-t_temp_time}")
        ...
        async def mcap_file_generator(chunk_size=4096*8):
            async with aiofiles.open(mcap_path, "rb") as mcap_file:
                while True:
                    chunk = await mcap_file.read(chunk_size)
                    if not chunk:
                        break
                    yield chunk
