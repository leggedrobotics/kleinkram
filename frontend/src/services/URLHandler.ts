...
export class QueryURLHandler extends QueryHandler {
	...
	constructor(
		...
		super(page, take, sortBy, descending, project_uuid, mission_uuid, search_params, file_type);
		if (router) {
	...
	readURL(route: RouteLocationNormalizedLoaded){
		...
		if(route.query.page) this.page = parseInt(route.query.page as string); else this.page = DEFAULT_PAGINATION.page;
		if(route.query.rowsPerPage) this.take = parseInt(route.query.rowsPerPage as string); else this.take = DEFAULT_PAGINATION.rowsPerPage;
		if(route.query.sortBy) this.sortBy = route.query.sortBy as string; else this.sortBy = DEFAULT_SORT.sortBy;
		if(route.query.descending) this.descending = route.query.descending === 'true'; else this.descending = DEFAULT_SORT.descending;
		if(route.query.project_uuid) this.project_uuid = route.query.project_uuid as string; else this.project_uuid = undefined;
		if(route.query.mission_uuid) this.mission_uuid = route.query.mission_uuid as string; else this.mission_uuid = undefined;
		if(route.query.name) this.search_params = {name: route.query.name as string}; else this.search_params = DEFAULT_SEARCH;
		if(route.query.file_type) this.file_type = route.query.file_type as FileType; else this.file_type = DEFAULT_FILE_TYPE;
	}
	...
	writeURL(){
		...
		const newQuery = {
			page: this.page !== DEFAULT_PAGINATION.page ? this.page.toString(): undefined,
			rowsPerPage: this.take !== DEFAULT_PAGINATION.rowsPerPage ? this.take.toString(): undefined,
			sortBy:this.sortBy !== DEFAULT_SORT.sortBy ? this.sortBy: undefined,
			descending: this.descending !== DEFAULT_SORT.descending ? this.descending.toString(): undefined,
			project_uuid: this.project_uuid || undefined,
			mission_uuid: this.mission_uuid || undefined,
			name: this.search_params?.name || undefined,
			file_type: this.project_uuid && this.mission_uuid ? this.file_type || undefined : undefined
		};
