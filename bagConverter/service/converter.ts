  ...
  for (const def of definitionArr) {
    ...
    for (const field of def.definitions) {
      if (field.isConstant === true) {
        fields.push(
          `// ${field.type} ${field.name} = ${field.valueText ?? field.value ?? ''}`,
        );
        continue;
      }
      ...
      if (
        field.isArray === true &&
        (field.type === 'uint8' || field.type === 'int8')
      ) {
...
export async function convert(
  ...
  for (const result of readResults) {
    ...
    try {
      const rosMsg = convertTypedArrays(
